...
public class BedwarsListeners implements Listener {
    ...
    public void onInteract(PlayerInteractEvent event){
        ...
        if(player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server1/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server2/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server3/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server4/server/")){
            if(event.getClickedBlock()!=null){
                ...
                if(event.getClickedBlock().getType()==Material.GLOW_ITEM_FRAME){
                    event.setCancelled(true);
                }
                if(event.getClickedBlock().getType()==Material.SPRUCE_BUTTON){
                    event.setCancelled(true);
                }
            }
        }
    ...
    public void onBreakBlock(BlockBreakEvent event){
        Player player = event.getPlayer();
        if(player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server1/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server2/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server3/server/")||player.getWorld().getName().equalsIgnoreCase("GameServer/Bedwars/Server4/server/")){
            if(!(event.getBlock().getType()==Material.RED_BED||event.getBlock().getType()==Material.CACTUS||event.getBlock().getType()==Material.DEAD_BUSH||event.getBlock().getType()==Material.YELLOW_BED||event.getBlock().getType()==Material.BLUE_BED||event.getBlock().getType()==Material.GREEN_BED||event.getBlock().getType()==Material.RED_WOOL||event.getBlock().getType()==Material.BLUE_WOOL||event.getBlock().getType()==Material.GREEN_WOOL||event.getBlock().getType()==Material.YELLOW_WOOL||event.getBlock().getType()==Material.STONE||event.getBlock().getType()==Material.BRICKS||event.getBlock().getType()==Material.END_STONE||event.getBlock().getType()==Material.LIGHT_WEIGHTED_PRESSURE_PLATE||event.getBlock().getType()==Material.YELLOW_CANDLE||event.getBlock().getType()==Material.YELLOW_CARPET||event.getBlock().getType()==Material.GRAY_CANDLE||event.getBlock().getType()==Material.LIGHT_GRAY_CARPET||event.getBlock().getType()==Material.STONE_PRESSURE_PLATE||event.getBlock().getType()==Material.ACACIA_PRESSURE_PLATE||event.getBlock().getType()==Material.BROWN_CARPET||event.getBlock().getType()==Material.BROWN_CANDLE||event.getBlock().getType()==Material.COBWEB)){
                event.setCancelled(true);
            }
    ...
    public boolean existBed(Player player, int serverMap){
        ...
        if(serverMap==3){
            ...
            if(team!=null){
                if(team.equalsIgnoreCase("red")){
                    if(player.getWorld().getBlockAt(5,-1,1).getType()==Material.RED_BED) return true;
                    return false;
                }else if(team.equalsIgnoreCase("blue")){
                    if(player.getWorld().getBlockAt(88,-1,-106).getType()==Material.BLUE_BED) return true;
                    return false;
                }else if(team.equalsIgnoreCase("yellow")){
                    if(player.getWorld().getBlockAt(-19,-1,-189).getType()==Material.YELLOW_BED) return true;
                    return false;
                }else if(team.equalsIgnoreCase("green")){
                    if(player.getWorld().getBlockAt(-102,-1,-82).getType()==Material.GREEN_BED) return true;
                    return false;
                }
            }
        }
